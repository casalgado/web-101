...
// Filter
// function(element, index, array) { return bool}

console.log(Number("6"));

function isExpensive(order) {
  return order.total > 30000;
}

// console.log(orders.filter(isExpensive));

// Map
// function(element, index, array) { return new_element }

function onlyProducts(order) {
  return { product: order.name };
}

// console.log(orders.map(onlyProducts));

// Reduce
// function(accumulator, currentValue, currentIndex) => {}

function sumOfTotals(accumulator, order) {
  return accumulator + order.total;
}

// console.log(orders.reduce(sumOfTotals, 0));

function totalsPerProduct(totals_object, order) {
  if (totals_object[order.name] == null) {
    totals_object[order.name] = 0;
  }
  totals_object[order.name] += Number(order.quantity);
  return totals_object;
}

console.log(orders.reduce(totalsPerProduct, {}));
